...
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("IMAGINWEB_SMS_subscr_tab_subscriber"), "ICON" => "main_user_edit", "TITLE" => GetMessage("IMAGINWEB_SMS_subscr_tab_subscriber_title")),
	array("DIV" => "edit2", "TAB" => GetMessage("IMAGINWEB_SMS_subscr_tab_subscription"), "ICON" => "main_user_edit", "TITLE" => GetMessage("IMAGINWEB_SMS_subscr_tab_subscription_title")),
);
...
if($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT >= "W" && check_bitrix_sessid()){
	...
	if($res){
		...
		if($e = $APPLICATION->GetException()){
			$message = new CAdminMessage(GetMessage("IMAGINWEB_SMS_subs_save_error"), $e);
		}
		$bVarsFromForm = true;
	}
}
...
$APPLICATION->SetTitle(($ID > 0 ? GetMessage("IMAGINWEB_SMS_subscr_title_edit").$ID : GetMessage("IMAGINWEB_SMS_subscr_title_add")));

require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");

$aMenu = array(
	array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_subscr_list_text"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_subscr_list"),
...
if($ID>0)
{
	...
	$aMenu[] = array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_subscr_add_text"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_subscr_mnu_add"),
		"LINK"=>"imaginweb.sms_subscr_edit.php?lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_subscr_del_text"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_subscr_mnu_del"),
		"LINK"=>"javascript:if(confirm('".GetMessage("IMAGINWEB_SMS_subscr_mnu_del_conf")."'))window.location='imaginweb.sms_subscr_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
}
...
if($_REQUEST["mess"] == "ok" && $ID>0)
	CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("IMAGINWEB_SMS_subs_saved"), "TYPE"=>"OK"));
if($message)
	echo $message->Show();
?>
...
?>
	<?if($ID > 0):?>
		<tr>
			<td><?=GetMessage("IMAGINWEB_SMS_subscr_date_add");?></td>
			<td><?=$str_DATE_INSERT;?></td>
		</tr>
		<?if($str_DATE_UPDATE <> ""):?>
			<tr>
				<td><?=GetMessage("IMAGINWEB_SMS_subscr_date_upd");?></td>
				<td><?=$str_DATE_UPDATE;?></td>
			</tr>
		<?endif?>
	<?endif?>
	<tr>
		<td width="40%"><?=GetMessage("IMAGINWEB_SMS_subscr_anonym");?></td>
	...
	<tr>
		<td><?=GetMessage("IMAGINWEB_SMS_subscr_user");?></td>
		<td>
		<?
		$sUser = "";
	...
	<tr>
		<td><?=GetMessage("IMAGINWEB_SMS_subscr_active");?></td>
		<td><input type="checkbox" name="ACTIVE" value="Y"<?if($str_ACTIVE == "Y") echo " checked";?>></td>
	</tr>
	<tr>
		<td><span class="required">*</span><?=GetMessage("IMAGINWEB_SMS_subscr_number");?></td>
...
?>
	<tr>
		<td width="40%"><?=GetMessage("IMAGINWEB_SMS_subscr_rub");?></td>
		<td width="60%">
		<?
			if($bVarsFromForm)
