...
$aTabs = array(
	array(
		"DIV" => "edit1",
		"TAB" => GetMessage("IMAGINWEB_SMS_post_subscribers"),
		"ICON"=>"main_user_edit",
		"TITLE"=>GetMessage("IMAGINWEB_SMS_post_tab_title"),
	),
);
...
?>
	<tr class="heading">
		<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_post_search_rub")?></td>
	</tr>
	<tr>
		<td width="40%"><?=GetMessage("IMAGINWEB_SMS_post_rub")?>:</td>
		<td width="60%">
	...
	<tr class="heading">
		<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_post_search_users")?></td>
	</tr>
	<tr>
		<td><?=GetMessage("IMAGINWEB_SMS_post_group")?></td>
		<td>
	...
	<tr class="heading">
		<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_post_search_filter")?></td>
	</tr>
	<tr>
...
$tabControl->Buttons();
?>
<input type="submit" name="search" value="<?=GetMessage("IMAGINWEB_SMS_post_search")?>">
<input type="reset" name="Reset" value="<?=GetMessage("IMAGINWEB_SMS_post_reset")?>">
...
if(count($aPhone) > 0):?>
	<h2><?=GetMessage("IMAGINWEB_SMS_post_result")?></h2>
	<p class="imaginweb.sms_border">
		<?=implode(", ",$aPhone)?>
	</p>
	<p><?=GetMessage("IMAGINWEB_SMS_post_total")?> <b><?=count($aPhone);?></b></p><?
else:
	CAdminMessage::ShowMessage(GetMessage("IMAGINWEB_SMS_post_notfound"));
endif;?>
<script>
<!--
function SetValues()
...
<input title="<?=GetMessage("IMAGINWEB_SMS_post_search_set_title")?> (<?=count($aPhone);?>)"  type="button" name="Set" value="<?=GetMessage("IMAGINWEB_SMS_post_set")?>" OnClick="SetValues()">
<input type="button" name="Close" value="<?=GetMessage("IMAGINWEB_SMS_post_cancel")?>" OnClick="window.close()">
</form>
<?=BeginNote();?>
<?=GetMessage("IMAGINWEB_SMS_post_search_note")?>
<?=EndNote();?>
