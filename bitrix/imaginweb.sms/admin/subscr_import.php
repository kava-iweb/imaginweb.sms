...
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("IMAGINWEB_SMS_imp_import_tab"), "ICON"=>"main_user_edit", "TITLE"=>GetMessage("IMAGINWEB_SMS_imp_import_tab_title")),
);
...
if($REQUEST_METHOD == "POST" && !empty($Import) && $POST_RIGHT >= "W" && check_bitrix_sessid()){
	...
	if(!empty($_FILES["ADDR_FILE"]["tmp_name"])){
		if((integer)$_FILES["ADDR_FILE"]["error"] <> 0){
			$arError[] = array("id"=>"ADDR_FILE", "text"=>GetMessage("IMAGINWEB_SMS_subscr_imp_err1")." (".GetMessage("IMAGINWEB_SMS_subscr_imp_err2")." ".$_FILES["ADDR_FILE"]["error"].")");
		}
		else{
	...
	foreach($aPhone as $phone => $temp){
		...
		/* if($USER_TYPE == "U"){
			...
			if($USER_ID = $user->Add($arUserFields)){
				...
				if($SEND_REG_INFO == "Y")
					$user->SendUserInfo($USER_ID, $LID, GetMessage("IMAGINWEB_SMS_subscr_send_info"));
			}
			else{
...
$APPLICATION->SetTitle(GetMessage("IMAGINWEB_SMS_imp_title"));

require_once($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");

if(count($arError)>0){
	$e = new CAdminException($arError);
	$message = new CAdminMessage(GetMessage("IMAGINWEB_SMS_imp_error"), $e);
	echo $message->Show();
}
?>
<?if($bShowRes):?>
	<p class="text"><b><?=GetMessage("IMAGINWEB_SMS_imp_results")?></b><br>
	<?=GetMessage("IMAGINWEB_SMS_imp_results_total")?> <b><?=count($aPhone)?></b><br>
	<?=GetMessage("IMAGINWEB_SMS_imp_results_added")?> <font class="<?=($nSuccess==0? "required":"pointed")?>"><b><?=$nSuccess?></b></font><br>
	<?=GetMessage("IMAGINWEB_SMS_imp_results_err")?> <font class="<?=($nError>0? "required":"pointed")?>"><b><?=$nError?></b></font>
	</p>
<?endif;?>
...
?>
	<tr class="heading">
		<td colspan="2"><?=GetMessage("IMAGINWEB_SMS_imp_delim")?></td>
	</tr>
	<tr>
		<td width="40%"><?=GetMessage("IMAGINWEB_SMS_imp_file")?></td>
		<td width="60%"><input type=file name="ADDR_FILE" size=30></td>
	</tr>
	<tr>
		<td><?=GetMessage("IMAGINWEB_SMS_imp_list")?></td>
		<td><textarea name="ADDR_LIST" rows=10 cols=45></textarea></td>
	</tr>
	<tr>
		<td><?=GetMessage("IMAGINWEB_SMS_imp_subscr")?></td>
		<td><?
...
$tabControl->Buttons();
?>
<input<?if($POST_RIGHT<"W") echo " disabled";?> class="button" type="submit" name="Import" value="<?=GetMessage("IMAGINWEB_SMS_imp_butt")?>">
