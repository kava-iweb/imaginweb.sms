...
$aTabs = array(
	array("DIV" => "edit1", "TAB" => GetMessage("IMAGINWEB_SMS_rub_tab_rubric"), "ICON" => "main_user_edit", "TITLE" => GetMessage("IMAGINWEB_SMS_rub_tab_rubric_title"))
);
...
if($REQUEST_METHOD == "POST" && ($save != "" || $apply != "") && $POST_RIGHT == "W" && check_bitrix_sessid()){
	...
	if($res){
		...
		if($e = $APPLICATION->GetException()){
			$message = new CAdminMessage(GetMessage("IMAGINWEB_SMS_rub_save_error"), $e);
		}
		$bVarsFromForm = true;
	}
...
$APPLICATION->SetTitle(($ID > 0 ? GetMessage("IMAGINWEB_SMS_rub_title_edit").$ID : GetMessage("IMAGINWEB_SMS_rub_title_add")));
require($_SERVER["DOCUMENT_ROOT"]."/bitrix/modules/main/include/prolog_admin_after.php");

$aMenu = array(
	array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_rub_list"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_rub_list_title"),
...
if($ID > 0){
	...
	$aMenu[] = array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_rub_add"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_rubric_mnu_add"),
		"LINK"=>"imaginweb.sms_rubric_edit.php?lang=".LANG,
		"ICON"=>"btn_new",
	);
	$aMenu[] = array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_rub_delete"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_rubric_mnu_del"),
		"LINK"=>"javascript:if(confirm('".GetMessage("IMAGINWEB_SMS_rubric_mnu_del_conf")."'))window.location='imaginweb.sms_rubric_admin.php?ID=".$ID."&action=delete&lang=".LANG."&".bitrix_sessid_get()."';",
		"ICON"=>"btn_delete",
	);
	/* $aMenu[] = array("SEPARATOR"=>"Y");
	$aMenu[] = array(
		"TEXT"=>GetMessage("IMAGINWEB_SMS_rub_check"),
		"TITLE"=>GetMessage("IMAGINWEB_SMS_rubric_mnu_check"),
...
if($_REQUEST["mess"] == "ok" && $ID > 0){
	CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("IMAGINWEB_SMS_rub_saved"), "TYPE"=>"OK"));
}
if($message){
	echo $message->Show();
}
