...
if($_REQUEST["action"] == "js_send" && check_bitrix_sessid()){
	...
	if($arPosting){
				...
				if($arPosting["AUTO_SEND_TIME"] != ""){
					if(COption::GetOptionString("imaginweb.sms", "subscribe_auto_method") !== "cron"){
						...
						if(!$rsAgents->Fetch()){
							...
							CAdminMessage::ShowMessage(array("MESSAGE" => GetMessage("IMAGINWEB_SMS_posting_agent_submitted"), "TYPE" => "OK"));
						}
					}
					else{
						CAdminMessage::ShowMessage(array("MESSAGE" => GetMessage("IMAGINWEB_SMS_posting_cron_setup"), "TYPE" => "OK"));
					}
					?>
					<script>
		...
		elseif($arPosting["STATUS"] == "S" || $arPosting["STATUS"] == "E"){
			CAdminMessage::ShowMessage(array("MESSAGE"=>GetMessage("IMAGINWEB_SMS_post_send_ok"), "TYPE"=>"OK"));
			?>
			<script>
				<?=$sTableID?>.GetAdminList('<?=$APPLICATION->GetCurPage();?>?lang=<?=LANGUAGE_ID?>');
			</script><?
			...
			CAdminMessage::ShowMessage(GetMessage("IMAGINWEB_SMS_POST_ADM_POST_NOT_FOUND"));
		}
	}
	else{
		CAdminMessage::ShowMessage(GetMessage("IMAGINWEB_SMS_POST_ADM_POST_NOT_FOUND"));
...
function CheckFilter(){
	...
	if(strlen(trim($find_timestamp_1)) > 0 || strlen(trim($find_timestamp_2)) > 0){
		...
		if (!$date1_stm && strlen(trim($find_timestamp_1))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_TIMESTAMP_FROM"));
		else $date_1_ok = true;
		if (!$date2_stm && strlen(trim($find_timestamp_2))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_TIMESTAMP_TILL"));
		elseif ($date_1_ok && $date2_stm <= $date1_stm && strlen($date2_stm)>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_FROM_TILL_TIMESTAMP"));
	}
	if(strlen(trim($find_date_sent_1))>0 || strlen(trim($find_date_sent_2))>0){
		...
		if (!$date1_stm && strlen(trim($find_date_sent_1))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_DATE_SENT_FROM"));
		else $date_1_ok = true;
		if (!$date2_stm && strlen(trim($find_date_sent_2))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_DATE_SENT_TILL"));
		elseif ($date_1_ok && $date2_stm <= $date1_stm && strlen($date2_stm)>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_FROM_TILL_DATE_SENT"));
	}
	if(strlen(trim($find_auto_send_time_1))>0 || strlen(trim($find_auto_send_time_2))>0){
		...
		if (!$date1_stm && strlen(trim($find_auto_send_time_1))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_DATE_AUTOSEND_FROM"));
		else $date_1_ok = true;
		if (!$date2_stm && strlen(trim($find_auto_send_time_2))>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_WRONG_DATE_AUTOSEND_TILL"));
		elseif ($date_1_ok && $date2_stm <= $date1_stm && strlen($date2_stm)>0)
			$lAdmin->AddFilterError(GetMessage("IMAGINWEB_SMS_POST_FROM_TILL_DATE_AUTOSEND"));
	}
...
if($lAdmin->EditAction() && $POST_RIGHT == "W")
{
	foreach($FIELDS as $ID=>$arFields)
	{
		...
		{
			$lAdmin->AddUpdateError(GetMessage("IMAGINWEB_SMS_post_save_err").$ID.": ".$ob->LAST_ERROR, $ID);
			$DB->Rollback();
		}
		$DB->Commit();
	}
...
if(($arID = $lAdmin->GroupAction()) && $POST_RIGHT=="W")
{
	...
	foreach($arID as $ID)
	{
		...
		case "delete":
			...
			{
				$DB->Rollback();
				$lAdmin->AddGroupError(GetMessage("IMAGINWEB_SMS_post_del_err"), $ID);
			}
			$DB->Commit();
			break;
		case "stop":
...
$lAdmin->AddHeaders(array(
	...
	array(
		"id"		=>"TIMESTAMP_X",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_updated"),
		"sort"		=>"timestamp",
		"default"	=>true,
	),
	array(
		"id"		=>"SUBJECT",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_subj"),
		"sort"		=>"subject",
		"default"	=>true,
	),
	/* array(
		"id"		=>"BODY_TYPE",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_body_type"),
		"sort"		=>"body_type",
		"default"	=>true,
	), */
	array(
		"id"		=>"STATUS",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_stat"),
		"sort"		=>"status",
		"default"	=>true,
	),
	array(
		"id"		=>"DATE_SENT",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_sent"),
		"sort"		=>"date_sent",
		"default"	=>true,
	),
	array(
		"id"		=>"SENT_TO",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_report"),
		"sort"		=>false,
		"default"	=>false,
	),
	array(
		"id"		=>"FROM_FIELD",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_from"),
		"sort"		=>"from_field",
		"default"	=>false,
	),
	array(
		"id"		=>"TO_FIELD",
		"content"	=>GetMessage("IMAGINWEB_SMS_post_to"),
		"sort"		=>"to_field",
		"default"	=>false,
	),
));
...
$rsData->NavStart();
$lAdmin->NavText($rsData->GetNavPrint(GetMessage("IMAGINWEB_SMS_post_nav")));

while($arRes = $rsData->NavNext(true, "f_")):
	$row =& $lAdmin->AddRow($f_ID, $arRes);
	$row->AddViewField("SUBJECT", '<a href="imaginweb.sms_posting_edit.php?ID='.$f_ID.'&amp;lang='.LANG.'" title="'.GetMessage("IMAGINWEB_SMS_post_act_edit").'">'.$f_SUBJECT.'</a>');
	...
	$row->AddViewField("SENT_TO", "[&nbsp;<a href=\"javascript:void(0)\" OnClick=\"jsUtils.OpenWindow('imaginweb.sms_posting_bcc.php?ID=".$f_ID."&lang=".LANG."', 600, 500);\">".GetMessage("IMAGINWEB_SMS_POST_SHOW_LIST")."</a>&nbsp;]");
	...
	if(($f_STATUS != "P") && $POST_RIGHT == "W")
		$arActions[] = array(
			"ICON"=>"edit",
			"DEFAULT"=>true,
			"TEXT"=>GetMessage("IMAGINWEB_SMS_post_act_edit"),
			"ACTION"=>$lAdmin->ActionRedirect("imaginweb.sms_posting_edit.php?ID=".$f_ID)
		);
	$arActions[] = array(
			"ICON"=>"copy",
			"TEXT"=>GetMessage("IMAGINWEB_SMS_posting_copy_link"),
			"ACTION"=>$lAdmin->ActionRedirect("imaginweb.sms_posting_edit.php?ID=".$f_ID."&amp;action=copy")
	);
	if(($f_STATUS != "P") && $POST_RIGHT == "W")
		$arActions[] = array(
			"ICON"=>"delete",
			"TEXT"=>GetMessage("IMAGINWEB_SMS_post_act_del"),
			"ACTION"=>"if(confirm('".GetMessage("IMAGINWEB_SMS_post_act_del_conf")."')) ".$lAdmin->ActionDoGroup($f_ID, "delete")
		);
	...
	if($f_STATUS == "D" && $POST_RIGHT == "W")
		$arActions[] = array(
			"ICON"=>"",
			"TEXT"=>GetMessage("IMAGINWEB_SMS_post_act_send"),
			"ACTION"=>"if(confirm('".GetMessage("IMAGINWEB_SMS_post_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=send&lang=".LANG."&".bitrix_sessid_get()."'"
		);
	if($f_STATUS == "W" && $POST_RIGHT == "W")
		$arActions[] = array(
			"ICON"=>"",
			"TEXT"=>GetMessage("IMAGINWEB_SMS_posting_continue_act"),
			"ACTION"=>"if(confirm('".GetMessage("IMAGINWEB_SMS_posting_continue_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=send&lang=".LANG."&".bitrix_sessid_get()."'"
		);
	if($f_STATUS == "P" && $POST_RIGHT == "W")
		$arActions[] = array(
			"ICON"=>"",
			"TEXT"=>GetMessage("IMAGINWEB_SMS_posting_stop_act"),
			"ACTION"=>"if(confirm('".GetMessage("IMAGINWEB_SMS_posting_stop_conf")."')) window.location='".$APPLICATION->GetCurPage()."?ID=".$f_ID."&action=stop&lang=".LANG."&".bitrix_sessid_get()."'"
		);
...
$aContext = array(
	array(
		...
		"TITLE"=>GetMessage("IMAGINWEB_SMS_POST_ADD_TITLE"),
		"ICON"=>"btn_new",
	),
);
...
$APPLICATION->SetTitle(GetMessage("IMAGINWEB_SMS_post_title"));
...
$oFilter = new CAdminFilter(
	$sTableID."_filter",
	array(
		"id" => GetMessage("IMAGINWEB_SMS_POST_F_ID"),
		"timestamp" => GetMessage("IMAGINWEB_SMS_POST_F_TIMESTAMP"),
		"date_sent" => GetMessage("IMAGINWEB_SMS_POST_F_DATE_SENT"),
		"auto_send_time" => GetMessage("IMAGINWEB_SMS_POST_F_AUTO_SEND_TIME"),
		"status" => GetMessage("IMAGINWEB_SMS_POST_F_STATUS"),
		"from" => GetMessage("IMAGINWEB_SMS_POST_F_FROM"),
		"to" => GetMessage("IMAGINWEB_SMS_POST_F_TO"),
		"subject" => GetMessage("IMAGINWEB_SMS_POST_F_SUBJECT"),
		/* "body_type" => GetMessage("IMAGINWEB_SMS_POST_F_BODY_TYPE"), */
		"body" => GetMessage("IMAGINWEB_SMS_POST_F_BODY"),
		"rubric" => GetMessage("IMAGINWEB_SMS_POST_F_RUBRIC"),
	)
);
?>
...
?>
<tr>
	<td><b><?=GetMessage("IMAGINWEB_SMS_POST_FIND")?>:</b></td>
	<td>
		<input type="text" size="25" name="find" value="<?=htmlspecialchars($find)?>" title="<?=GetMessage("IMAGINWEB_SMS_POST_FIND_TITLE")?>">
		<?
		$arr = array(
			"reference" => array(
				GetMessage("IMAGINWEB_SMS_POST_F_SUBJECT"),
				GetMessage("IMAGINWEB_SMS_POST_F_ID"),
				GetMessage("IMAGINWEB_SMS_POST_F_STATUS"),
			),
			"reference_id" => array(
				"subject",
				"id",
...
<tr>
	<td><?=GetMessage("IMAGINWEB_SMS_POST_F_ID")?>:</td>
	<td>
...
if($_REQUEST['action'] == "send"):
	...
	<div id="progress_monitor" style="display:none">
		<p><?=GetMessage("IMAGINWEB_SMS_POST_ADM_SENDING_NOTE_LINE1")?><br>
		<?=GetMessage("IMAGINWEB_SMS_POST_ADM_SENDING_NOTE_LINE2")?></p>
		<table border="0" cellpadding="0" width="350" cellspacing="0"><tr><td>
			...
			<p>
			<?=GetMessage("IMAGINWEB_SMS_posting_addr_processed")?> <span id="progress_cnt">0</span> <?=GetMessage("IMAGINWEB_SMS_posting_addr_of")?> <span id="progress_total">0</span> (<span id="progress_perc">0</span>%)<br>
			<?=GetMessage("IMAGINWEB_SMS_POST_ADM_WITH_ERRORS")?>: <span id="progress_error">0</span>.
			</p>
		</td></tr></table>
	</div>
	<div id="progress_message">
	<input type="button" value="<?=GetMessage("IMAGINWEB_SMS_POST_ADM_BTN_STOP")?>" id="btn_stop" OnClick="Stop()"  style="display:none">
	<input type="button" value="<?=GetMessage("IMAGINWEB_SMS_posting_continue_button")?>" id="btn_cont" OnClick="Cont()" disabled  style="display:none">
	</div>
	<script>
		var stop = false;
		var _sent = 0;
