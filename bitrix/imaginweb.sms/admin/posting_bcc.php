...
$lAdmin->AddHeaders(array(
	array(
		"id" => "PHONE",
		"content" => GetMessage("IMAGINWEB_SMS_POST_PHONE"),
		"default" => true,
	),
	array(
		"id" => "SUBSCRIPTION_ID",
		"content" => GetMessage("IMAGINWEB_SMS_POST_SUBSCRIPTION_ID"),
		"default" => true,
		"align" => "right",
	),
	array(
		"id" => "USER_ID",
		"content" => GetMessage("IMAGINWEB_SMS_POST_USER_ID"),
		"default" => true,
		"align" => "right",
	),
));
...
while($arRes = $rsData->NavNext(true, "f_"))
{
	...
	if($f_SUBSCRIPTION_ID > 0)
	{
		...
		$ar = $rs->Fetch();
		if(!$ar)
			$row->AddViewField("SUBSCRIPTION_ID", $f_SUBSCRIPTION_ID.' ('.GetMessage("IMAGINWEB_SMS_POST_SUBSCR_DELETED").')');
		elseif($ar["ACTIVE"]=="N")
			$row->AddViewField("SUBSCRIPTION_ID", '<a target="_blank" href="subscr_edit.php?lang='.LANGUAGE_ID.'&amp;ID='.$f_SUBSCRIPTION_ID.'">'.$f_SUBSCRIPTION_ID.'</a> ('.GetMessage("IMAGINWEB_SMS_POST_SUBSCR_INACTIVE").')');
		else
...
$lAdmin->AddFooter(
	array(
		array("title"=>GetMessage("IMAGINWEB_SMS_post_total"), "value"=>$rsData->SelectedRowsCount()),
		array("counter"=>true, "title"=>GetMessage("MAIN_ADMIN_LIST_CHECKED"), "value"=>"0"),
	)
);
$lAdmin->AddGroupActionTable(Array(
	"inactive"=>GetMessage("IMAGINWEB_SMS_POST_GROUP_ACTION_INACTIVE"),
	"sudelete"=>GetMessage("IMAGINWEB_SMS_POST_GROUP_ACTION_DELETE"),
));
...
?>
<tr>
	<td><?=GetMessage("IMAGINWEB_SMS_POST_STATUS_ID")?>:</td>
	<td>
		<?
		$arr = array(
			"reference" => array(
				GetMessage("IMAGINWEB_SMS_POST_STATUS_ID_ERROR"),
				GetMessage("IMAGINWEB_SMS_POST_STATUS_ID_SUCCESS"),
			),
			"reference_id" => array(
				"E",
				"N",
